...
/*BOARD_HAVE_BLUETOOTH_RTK_VR begin*/
#ifdef  BLUETOOTH_RTK_VR
static void btif_voice_ctrl_cb(tUIPC_CH_ID ch_id, tUIPC_EVENT event);
static BOOLEAN HH_task_running = FALSE;
#endif
/*BOARD_HAVE_BLUETOOTH_RTK_VR end*/
...
void btif_hh_remove_device(bt_bdaddr_t bd_addr)
{
    ...
    for (i = 0; i < BTIF_HH_MAX_ADDED_DEV; i++) {
        ...
        if (memcmp(&(p_added_dev->bd_addr),&bd_addr, 6) == 0) {
            ...
            BTIF_TRACE_WARNING1("p_added_dev->dev_handle = %d", p_added_dev->dev_handle);
            BTIF_TRACE_WARNING1("%s: to call btif_storage_remove_hid_info", __FUNCTION__);
...
/*BOARD_HAVE_BLUETOOTH_RTK_VR Begin*/
#ifdef BLUETOOTH_RTK_VR
            if (p_data->conn.status == BTA_HH_RTKBT_VR_OK){
                BTIF_TRACE_WARNING0("cheng_@btif_hh_upstreams_evt begin to callback!");
                ...
                if (p_dev == NULL) {
                    ...
                    RTKBT_VR_stop_msbc_voice_rec();
                    break;
                }
                else{
                    if (memcmp(p_dev->bd_addr.address,p_data->conn.bda, BD_ADDR_LEN) == 0){
                        BTIF_TRACE_WARNING0("cheng_@btif_hh_upstreams_evt bdadress is the same");
                        HAL_CBACK(bt_hh_callbacks, connection_state_cb,&(p_dev->bd_addr), BTHH_CONN_STATE_RTKBT_VR_CONNECTED);
                        break;
                    }
                else{
                        BTIF_TRACE_ERROR0("BTA_HH_OPEN_EVT: Error, bdaddr is not the same!");
                        HAL_CBACK(bt_hh_callbacks, connection_state_cb,&(p_dev->bd_addr), BTHH_CONN_STATE_RTKBT_VR_CONNECTED);
                    }
                    break;
                }
...
/*BOARD_HAVE_BLUETOOTH_RTK_VR begin*/
#ifdef BLUETOOTH_RTK_VR
void RTKBT_VR_cmd_acknowledge(int status)
{
    UINT8 ack = status;

    APPL_TRACE_EVENT1("## voice ack status %d ##", status);

    /* acknowledge start request */
    UIPC_Send(UIPC_CH_ID_RTKBT_VR_CTRL, 0, &ack, 1);
}

static void btif_recv_voice_ctrl_data(void)
{
    UINT8 cmd[5];
    int n;

    n = UIPC_Read(UIPC_CH_ID_RTKBT_VR_CTRL, NULL, cmd, 4);
    ...
    APPL_TRACE_DEBUG0("receive voice-ctrl-cmd ");

    //receive start or stop cmd from service
    if((cmd[0] == 0xAA) && (cmd[1] == 0x55) && (cmd[2] == 0xCC) && (cmd[3] == 0x33)){
        APPL_TRACE_EVENT0("cheng_@ it is cmd send from service,the sysnc word is right!");
        UIPC_Read(UIPC_CH_ID_RTKBT_VR_CTRL, NULL, &cmd[4], 1);
    }
    else{
        //clear the memory
        UIPC_Ioctl(UIPC_CH_ID_RTKBT_VR_CTRL,UIPC_REQ_RX_FLUSH,NULL);
        APPL_TRACE_EVENT0("cheng_@ it is wrong cmd send from service");
    }

    switch(cmd[4])
    {
        case VOHOG_CTRL_CMD_START:
            APPL_TRACE_EVENT0("cheng_@start record!");
            APPL_TRACE_EVENT1("cheng_@msbc_rec_cb.p_dev_cb = %p",RTKBT_VR_get_dev_cb());
            if(RTKBT_VR_get_dev_cb()){
                RTKBT_VR_Audio_set_ok(TRUE);
                int i = 0;
                while(i<2000)
                {
                    if(RTKBT_VR_start_request_status() == TRUE)
                    {
                        APPL_TRACE_EVENT0("cheng_@start record request success!");
                        RTKBT_VR_cmd_acknowledge(VOHOG_CTRL_ACK_SUCCESS);
                        break;
                    }
                    i++;
                    usleep(1000);
                }
                if(!RTKBT_VR_start_request_status())
                {
                    APPL_TRACE_EVENT0("cheng_@send start record request failure!");
                    RTKBT_VR_stop_msbc_voice_rec();
                    RTKBT_VR_cmd_acknowledge(VOHOG_CTRL_ACK_FAILURE);
                }
            }
            else{
                APPL_TRACE_EVENT0("cheng_@not start send record!");
                RTKBT_VR_not_snd_stop_request(TRUE);
                RTKBT_VR_stop_msbc_voice_rec();
                RTKBT_VR_cmd_acknowledge(VOHOG_CTRL_ACK_FAILURE);
            }
        ...
        case VOHOG_CTRL_CMD_STOP:
            APPL_TRACE_EVENT0("cheng_@finish record!");
            RTKBT_VR_cmd_acknowledge(VOHOG_CTRL_ACK_SUCCESS);
            //stop record
            RTKBT_VR_stop_msbc_voice_rec();
        break;
        ...
        default:
            APPL_TRACE_ERROR0("cheng_@Unknown cmd!");
        break;
    }

    APPL_TRACE_EVENT0("cheng_@voice-ctrl-cmd DOWN!");
}
...
static void btif_voice_ctrl_cb(tUIPC_CH_ID ch_id, tUIPC_EVENT event)
{
    ...
    switch(event)
    {
        ...
        case UIPC_RX_DATA_EVT:
            APPL_TRACE_EVENT0("cheng_@btif_voice_ctrl_cb, the event is UIPC_RX_DATA_EVT");

        case UIPC_RX_DATA_READY_EVT:
            btif_recv_voice_ctrl_data();
            break;
