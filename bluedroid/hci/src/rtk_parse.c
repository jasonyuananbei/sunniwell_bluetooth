...
typedef struct _RTK_CONN_PROF {
    RT_LIST_ENTRY list;
    uint16_t handle;
    uint8_t type; // 0:l2cap, 1:sco/esco, 2:le
    uint8_t profile_bitmap;      //for coex , bit[0]: SCO , bit[1]: HID , bit[2]: A2DP , bit[3]: FTP/PAN/OPP , bit[4]: HOGP, bit[5]: VOICE ,bit[7:6]: reserved , 1:connect , 0:disconnect
    int8_t  profile_refcount[8]; //for coex, 0:SCO, 1:HID, 2:A2DP, 3:FTP/PAN/OPP, 4:hogp, 5:voice
}RTK_CONN_PROF, *PRTK_CONN_PROF;

//profile info for all
typedef struct _RTK_PROF
{
    ...
    uint8_t  profile_bitmap; //for coex , bit[0]: SCO , bit[1]: HID , bit[2]: A2DP , bit[3]: FTP/PAN/OPP , bit[4]: HOGP, bit[5]: VOICE ,bit[7:6]: reserved , 1:connect , 0:disconnect
    uint8_t  profile_status; //for coex , bit[0]: SCO , bit[1]: HID , bit[2]: A2DP , bit[3]: FTP/PAN/OPP , bit[4]: HOGP, bit[5]: VOICE ,bit[7:6]:reserved , 1:connect , 0:disconnect
    int8_t   profile_refcount[8];//for coex, 0:SCO, 1:HID, 2:A2DP, 3:FTP/PAN/OPP, 4:hogp, 5:voice
    uint8_t  ispairing;
...
typedef struct _HCI_LINKSTATUS_REPORT{
    uint8_t     cmd_index;
    uint8_t     cmd_length;
    uint8_t     link_status;
    uint8_t     retry_cnt;
...
typedef struct _HCI_RETURN_PARAMETER_MAILBOX_REGISTER{
    uint8_t  type;
    uint32_t offset;
    uint32_t value;
}HCI_RETURN_PARAMETER_MAILBOX_REGISTER;

typedef struct _HCI_EVENT_BT_INFO_CONTROL
{
    uint8_t     polling_enable;
    uint8_t     polling_time;
    uint8_t     autoreport_enable;
}HCI_EVENT_BT_INFO_CONTROL,*P_HCI_EVENT_BT_INFO_CONTROL;

RTK_PROF rtk_prof;
extern tHCI_IF *p_hci_if;
static pthread_mutex_t udpsocket_mutex = PTHREAD_MUTEX_INITIALIZER;
...
RTK_CONN_PROF* find_connection_by_handle(RTK_PROF* h5, uint16_t handle)
{
...
RTK_CONN_PROF* allocate_connection_by_handle(uint16_t handle)
{
	RTK_CONN_PROF * phci_conn = NULL;
	phci_conn = malloc(sizeof(RTK_CONN_PROF));
	if(phci_conn)
...
void init_connection_hash(RTK_PROF* h5)
{
	RT_LIST_HEAD* head = &h5->conn_hash;
	ListInitializeHeader(head);
}

void add_connection_to_hash(RTK_PROF* h5, RTK_CONN_PROF* desc)
{
	RT_LIST_HEAD* head = &h5->conn_hash;
	ListAddToTail(&desc->list, head);
}

void delete_connection_from_hash(RTK_CONN_PROF* desc)
{
	if (desc)
	{
...
void flush_connection_hash(RTK_PROF* h5)
{
	RT_LIST_HEAD* head = &h5->conn_hash;
...
void init_profile_hash(RTK_PROF* h5)
{
	RT_LIST_HEAD* head = &h5->profile_list;
	ListInitializeHeader(head);
}

uint8_t list_allocate_add(uint16_t handle, uint16_t psm, uint16_t dcid, uint16_t scid, RTK_PROF_INFO** ppprof_info)
{
	*ppprof_info = malloc(sizeof(RTK_PROF_INFO));
	if (NULL == *ppprof_info)
	{
	...
	(*ppprof_info)->handle = handle;
	(*ppprof_info)->psm = psm;
	(*ppprof_info)->scid = scid;
	(*ppprof_info)->dcid = dcid;

	ListAddToTail(&((*ppprof_info)->list), &(rtk_prof.profile_list));

	return TRUE;
}

void delete_profile_from_hash(RTK_PROF_INFO* desc)
{
...
void flush_profile_hash(RTK_PROF* h5)
{
	RT_LIST_HEAD* head = &h5->profile_list;
	RT_LIST_ENTRY* iter = NULL, *temp = NULL;
...
uint8_t psm_need_catch(uint16_t psm)
{
	uint8_t Count = sizeof(psm_list)/sizeof(uint16_t);
	uint8_t i = 0;
...
RTK_PROF_INFO* find_profile_by_handle_scid(RTK_PROF* h5, uint16_t handle, uint16_t scid)
{
	RT_LIST_HEAD* head = &h5->profile_list;
	RT_LIST_ENTRY* iter = NULL;
	RTK_PROF_INFO* desc = NULL;
...
RTK_PROF_INFO* find_profile_by_handle_dcid(RTK_PROF* h5, uint16_t handle, uint16_t dcid)
{
	RT_LIST_HEAD* head = &h5->profile_list;
	RT_LIST_ENTRY* iter = NULL;
	RTK_PROF_INFO* desc = NULL;
...
RTK_PROF_INFO* find_profile_by_handle_dcid_scid(RTK_PROF* h5, uint16_t handle, uint16_t dcid, uint16_t scid)
{
	RT_LIST_HEAD* head = &h5->profile_list;
	RT_LIST_ENTRY* iter = NULL;
	RTK_PROF_INFO* desc = NULL;
...
void update_profile_connection(RTK_CONN_PROF * phci_conn, uint16_t psm, uint8_t is_add ,uint8_t connection_type)
{
	uint8_t need_update = FALSE;
	uint8_t profile_index;
    //connection_type:0:l2cap, 1:sco, 2:le, 3:3dd
	LogMsg("update_profile_connection: is_add=%d, psm=%x, connection_type=%d", is_add, psm, connection_type);
	if((is_add)&&(!connection_type))
	{
		if((PSM_AVDTP == psm) && (0 == rtk_prof.profile_refcount[profile_a2dp]))
		{
			rtk_prof.a2dp_packet_count = 0;
			start_a2dp_packet_count_timer();
		}
		else if((PSM_PAN==psm || PSM_RFCOMM==psm || PSM_OPP==psm || PSM_FTP==psm || PSM_BIP==psm) 
			&& (0 == rtk_prof.profile_refcount[profile_pan]))
		{
    ...
    if(is_add && (2 == connection_type) && 
        (0 == rtk_prof.profile_refcount[profile_hogp]) && (0 == rtk_prof.profile_refcount[profile_voice]))
    {
    ...
    {
        case 1:
            profile_index = profile_sco;
            break;
        case 2:
            profile_index = profile_hogp;
            break;
        case 0:
        {
            switch (psm)
		    {
			    ...
			    case PSM_HID:
                case PSM_HID_INT:
				    profile_index = profile_hid;
				    break;
			    case PSM_AVDTP:
				    profile_index = profile_a2dp;
				    break;
			    case PSM_PAN:
			    case PSM_OPP:
			    case PSM_FTP:
	...
	if(is_add)
	{
		...
		if(rtk_prof.profile_refcount[profile_index] == 1)
		{
			need_update = TRUE;
		...
		else if(phci_conn->profile_refcount[profile_index] > 0)
		{
	...
	else
	{
		...
		if(rtk_prof.profile_refcount[profile_index] == 0)
		{
            ...
            if((profile_hogp == profile_index) && (rtk_prof.profile_refcount[profile_voice] == 0))
			{
				rtk_prof.hogp_packet_count = 0;
        ...
        if(rtk_prof.profile_refcount[profile_index] < 0)
        {
            ALOGE("error, rtk_prof.profile_refcount[%x] < 0", profile_index);
            rtk_prof.profile_refcount[profile_index] = 0;
        }
        ...
        {
            ALOGE("error, phci_conn->profile_refcount[%x] < 0", profile_index);
            phci_conn->profile_refcount[profile_index] = 0;
        }
	}

    //if le, check if voice over hogp needed
    if((2 == connection_type) && (psm & 0x0002))
    {
        if(is_add)
        {
            rtk_prof.profile_refcount[profile_voice]++;
            if(rtk_prof.profile_refcount[profile_voice] == 1)
            {
                need_update = TRUE;
                rtk_prof.profile_bitmap |= BIT(profile_voice);
            }

            if(0 == phci_conn->profile_refcount[profile_voice])
            {
                need_update = TRUE;
                phci_conn->profile_bitmap |= BIT(profile_voice);
                phci_conn->profile_refcount[profile_voice] = 1;
            }
            else if(phci_conn->profile_refcount[profile_voice] > 0)
            {
                phci_conn->profile_refcount[profile_voice]++;
            }
        }
        else
        {
            rtk_prof.profile_refcount[profile_voice]--;
            LogMsg("for test: --, rtk_prof.profile_refcount[5] = %x", rtk_prof.profile_refcount[profile_voice]);
            if(rtk_prof.profile_refcount[profile_voice] == 0)
            {
                ...
                //If profile does not exist, Status is meaningless
                rtk_prof.profile_status &= ~(BIT(profile_voice));
            ...
            if(rtk_prof.profile_refcount[profile_voice] < 0)
            {
                LogMsg("error, rtk_prof.profile_refcount[%x] < 0", profile_voice);
                rtk_prof.profile_refcount[profile_voice] = 0;
            }
            ...
            {
                ALOGE("error, phci_conn->profile_refcount[%x] < 0", profile_voice);
                phci_conn->profile_refcount[profile_voice] = 0;
            }
        }
    }
...
void update_hid_active_state(uint16_t handle, uint16_t interval)
{
	...
	RTK_CONN_PROF *phci_conn = find_connection_by_handle(&rtk_prof, handle);
...
uint8_t handle_l2cap_con_req(uint16_t handle, uint16_t psm, uint16_t scid, uint8_t direction)
{
	...
	if( PSM_SDP == psm )
	{
		LogMsg("handle_l2cap_con_req: Don't Process SDP");
		return Status;
	}

	if (!psm_need_catch(psm))
	{
		LogMsg("handle_l2cap_con_req: Not find this PSM(%x), Set to Default Index(PAN)", psm);
		psm = PSM_PAN;
	}
	...
	if(direction)//1: out
		Status = list_allocate_add(handle, psm, 0, scid, &prof_info);
	else // 0:in
		Status = list_allocate_add(handle, psm, scid, 0, &prof_info);
...
uint8_t handle_l2cap_con_rsp(uint16_t handle, uint16_t dcid, uint16_t scid, uint8_t direction, uint8_t result)
{
    ...
    if(!result)//success
	{
	    ...
	    RTK_CONN_PROF *phci_conn = find_connection_by_handle(&rtk_prof, handle);
		if(phci_conn)
	    	update_profile_connection(phci_conn, prof_info->psm, TRUE, 0);
    }
...
uint8_t handle_l2cap_discon_req(uint16_t handle, uint16_t dcid, uint16_t scid, uint8_t direction)
{
	...
	update_profile_connection(phci_conn, prof_info->psm, FALSE, 0);
...
void packets_count(uint16_t handle, uint16_t scid, uint16_t length, uint8_t direction)
{
    ...
    RTK_CONN_PROF* hci_conn = find_connection_by_handle(&rtk_prof, handle);
    if(NULL == hci_conn)
        return;
    ...
    if(0 == hci_conn->type)//l2cap
    {
	    ...
	    switch (prof_info->psm)
	    {
		    case PSM_AVDTP:
			    profile_type = profile_a2dp;
			    break;
		    case PSM_PAN:
		    case PSM_OPP:
		    case PSM_FTP:
	    ...
	    if((profile_type == profile_a2dp) && (length > 100))//avdtp media data
	    {
...
void rtk_notify_regester_to_wifi(uint8_t* reg_value)
{
    ...
    *p++ = para_length;
    ...
    HCI_RETURN_PARAMETER_MAILBOX_REGISTER *reg = (HCI_RETURN_PARAMETER_MAILBOX_REGISTER *)reg_value;
...
void rtk_handle_event_from_wifi(uint8_t* msg)
{
    ...
    if(event_code == 0xFE)
    {
        ...
        switch(extension_event)
        {
            ...
            {
                HCI_EVENT_BT_INFO_CONTROL*  info = (HCI_EVENT_BT_INFO_CONTROL*)p;
...
void rtk_parse_internal_event_intercept(uint8_t *p_msg)
{
	...
	if (event_code == HCI_INQUIRY_COMP_EVT)
	{
	...
	if (event_code == HCI_PIN_CODE_REQUEST_EVT)
	{
	...
	if (event_code == HCI_IO_CAPABILITY_REQUEST_EVT)
	{
		if(!rtk_prof.ispairing)
		{
			rtk_prof.ispairing = 1;
	...
	if (event_code == HCI_AUTHENTICATION_COMP_EVT)
	{
		if(rtk_prof.ispairing)
		{
	...
	if (event_code == HCI_LINK_KEY_NOTIFICATION_EVT)
	{
		if(rtk_prof.ispairing)
		{
			rtk_prof.ispairing = 0;
	...
	if (event_code == HCI_MODE_CHANGE_EVT)
	{
		uint8_t status = *p++;
		uint16_t mode_change_handle;
	...
	if (event_code == HCI_COMMAND_COMPLETE_EVT)
	{
		...
		if (opcode == HCI_PERIODIC_INQUIRY_MODE)
		{
			uint8_t status = *p++;
        ...
        if (opcode == 0xfc1b)
        {
            LogMsg("received event, ok to power off");
            poweroff_allowed = 1;
        }
	...
	if((event_code == HCI_CONNECTION_COMP_EVT) ||(event_code == HCI_ESCO_CONNECTION_COMP_EVT))
	{
		uint8_t status = 0;
		uint16_t handle = 0;
		status = *p++;
		...
		if(status == 0)
		{
			...
			if(hci_conn == NULL)
			{
				...
				if(hci_conn)
				{
                    ...
                    {
                        hci_conn->type = 1;
						update_profile_connection(hci_conn, 0, TRUE, 1);
                    }
                    else
                        hci_conn->type = 0;
				}
	...
	if(event_code == HCI_DISCONNECTION_COMP_EVT)
	{
		...
		if(status == 0)
		{
			...
			if(hci_conn)
			{
			    switch(hci_conn->type)
                {
                    case 0:
                    {
					    ...
					    LIST_FOR_EACH_SAFELY(iter, temp, &rtk_prof.profile_list)
					    {
						    ...
						    if ((handle == prof_info->handle) && prof_info->scid && prof_info->dcid)
						    {
								...
								update_profile_connection(hci_conn, prof_info->psm, FALSE, 0);
                                delete_profile_from_hash(prof_info);
						    }
					    }
					    pthread_mutex_unlock(&rtk_prof.profile_mutex);
                    ...
                    case 1:
                        update_profile_connection(hci_conn, 0, FALSE, hci_conn->type);
                        break;
                    case 2:
                        LogMsg("do not handle le disconnection here");
                        return;
                    default:
                        break;
                }                
	...
	if (event_code == HCI_VENDOR_SPECIFIC_EVT)
	{
	    uint16_t subcode;
...
void rtk_parse_command(uint8_t *pp)
{
    ...
    if((cmd == HCI_INQUIRY) ||(cmd == HCI_PERIODIC_INQUIRY_MODE))
	{
		if(!rtk_prof.isinquirying)
		{
	...
	if((cmd == HCI_INQUIRY_CANCEL) || (cmd == HCI_EXIT_PERIODIC_INQUIRY_MODE))
	{
		if(rtk_prof.isinquirying)
		{
			rtk_prof.isinquirying = 0;
	...
	if(cmd == HCI_ACCEPT_CONNECTION_REQUEST)
	{
		if(!rtk_prof.ispaging)
		{
			rtk_prof.ispaging = 1;
...
void rtk_add_le_profile(BD_ADDR bdaddr, uint16_t handle, uint8_t profile_map)
{
    ...
    if(hci_conn == NULL)
    {
        ...
        if(hci_conn)
        {
            ...
            memset(hci_conn->profile_refcount, 0, 8);
            hci_conn->type = 2;
		    update_profile_connection(hci_conn, le_prof, TRUE, 2);
        }
        else
        {
            ALOGE("HciConnAllocate fail");
...
void rtk_delete_le_profile(BD_ADDR bdaddr, uint16_t handle, uint8_t profile_map)
{
    ...
    if(hci_conn == NULL)
    {
        ALOGE("hci_conn not exist with handle %x", handle);
    }
    else
    {
        update_profile_connection(hci_conn, le_prof, FALSE, 2);
        delete_connection_from_hash(hci_conn);
    }
    pthread_mutex_unlock(&rtk_prof.profile_mutex);
}
