...
const int F5_value[8] = {0x00, 0x64, 0x3e, 0x00, 0x00, 0x00, 0x00, 0x00};
const int F11_value[8] = {0x00, 0x00, 0x44, 0x00, 0x00, 0x00, 0x00, 0x00};
...
#ifdef BLUETOOTH_RTK_VR
            APPL_TRACE_DEBUG2("%s, + p_dev_cb->disc_flag = %d", __FUNCTION__, p_cb->disc_flag );
            p_cb->disc_flag &= ~(BTA_HH_LE_SEC_CMPL | BTA_HH_LE_MTU_CMPL);
            APPL_TRACE_DEBUG2("%s, + p_dev_cb->disc_flag = %d", __FUNCTION__, p_cb->disc_flag );
#endif
...
/*BOARD_HAVE_BLUETOOTH_RTK_VR begin*/
#ifdef BLUETOOTH_RTK_VR
    RTKBT_VR_device_disconn(p_data->conn_id);
#endif
/*BOARD_HAVE_BLUETOOTH_RTK_VR end*/
...
/*BOARD_HAVE_BLUETOOTH_RTK_VR Begin*/
    ...
    if((p_rpt->rpt_id == 1) && (p_data->len == 8) && (remote_controller_id != 0)){
        ...
        while(i < p_data->len)
        {
             if(rtkbt_getRealRcuID(remote_controller_id) == RTKBT_RCUID_IFLYTEK) {
                 if(F5_value[i] == p_data->value[i]) {
                    APPL_TRACE_DEBUG1("F11-pdata is 0x%x", p_data->value[i]);
                    i++;
                 } else
                    break;
             } else {
                 if(F5_value[i] == p_data->value[i]) {
                    APPL_TRACE_DEBUG1("F5-pdata is 0x%x", p_data->value[i]);
                    i++;
                 } else
                    break;
             }
        }
        if(i == 8  && rtkbt_getRealRcuID(remote_controller_id) != RTKBT_RCUID_IFLYTEK)
        {
            UINT16 len;
            BT_HDR  *p_buf_voice;
        ...
        }else if (i == 8  && rtkbt_getRealRcuID(remote_controller_id) == RTKBT_RCUID_IFLYTEK)
		RTKBT_Iflytek_set_dev_cb(p_dev_cb);
	}

    if(p_rpt->rpt_id == RTKBT_INPUT_REPORT)
        process_id = 1;
    else if(p_rpt->rpt_id == rtk_voice_input_data)
        process_id = 2;
    else
        process_id = 0;
    ...
    switch(process_id)
    {
    //Report characteristic (Input Reports)
    //case RTKBT_INPUT_REPORT:
    case 1:
    if(p_rpt->rpt_id == RTKBT_INPUT_REPORT && p_rpt->rpt_type == RTKBT_INPUT_REPORT_TYPE)
    {
        /*UINT16 len;
        BT_HDR  *p_buf_voice;
        tBTA_HH_LE_RPT  *voice_output_p_rpt;
        ...
        APPL_TRACE_DEBUG0("cheng_@receive start record request!");
        if(p_dev_cb->state != BTA_HH_CONN_ST)
        {
            APPL_TRACE_DEBUG0("cheng_@ HH is not connected!");
            return;
        ...
        if(p_data->value[0] == 0x01){
            /*voice_output_r_type = RTKBT_OUTPUT_REPORT_TYPE;
            voice_output_rpt_id = RTKBT_OUTPUT_REPORT;
            len = 0x02;
            voice_output_p_rpt = bta_hh_le_find_rpt_by_idtype(p_dev_cb->hid_srvc[BTA_HH_LE_SRVC_DEF].report,
                                        BTA_HH_PROTO_RPT_MODE, voice_output_r_type, voice_output_rpt_id);
        ...
        {
            APPL_TRACE_DEBUG0("cheng_@ receive stop record response, for now we don't care about it!");
        }
        else
        {
            APPL_TRACE_DEBUG0("cheng_@ unknow input-ctrl data value ignore it!");
        }
    }
    break;

    //Report Reference(input data)
    case 2:
...
/*BOARD_HAVE_BLUETOOTH_RTK_COEX end*/
            ...
            p_buf_data->data = p_buf_data + 1;
            APPL_TRACE_DEBUG1("cheng_@ rpt_id = %d",*(p_buf_data->data));

            if(rtkbt_getRealRcuID(remote_controller_id) == RTKBT_RCUID_MDT){
                if(RTKBT_VR_msbc_voice_decode_MDT(p_buf_data))
                {
                ...
                else
		{
            ...
            APPL_TRACE_DEBUG0("cheng_@ unknow type voice data ignore it!");
        }
        break;
    default:
#endif
...
#ifdef BLUETOOTH_RTK
    if(!(p_rpt->rpt_id == RTKBT_IFLYTEK_INPUT_ACK || p_rpt->rpt_id == RTKBT_IFLYTEK_INPUT_DATA) ){
        if((p_dev_cb->dscp_info.vendor_id== RCU_VENDOR_ID)) {
...
void bta_hh_le_get_dscp_act(tBTA_HH_DEV_CB *p_cb)
{
    ...
    for (i = 0 ;i < BTA_HH_LE_HID_SRVC_MAX; i ++)
    {
	    ...
	    if(p_cb->dscp_info.vendor_id == 0x0416)
		RTKBT_Iflytek_set_dev_cb(p_cb);
        }
        else
            break;
    }
...
void bta_hh_le_remove_dev_bg_conn(tBTA_HH_DEV_CB *p_dev_cb)
{
    ...
    if (p_dev_cb->in_bg_conn)
    {
        ...
        APPL_TRACE_DEBUG1("%s to call BTA_GATTC_CancelOpen", __FUNCTION__);
...
#ifdef BLUETOOTH_RTK_VR
        case BTA_GATTC_EXCHANGE_MTU_CMPL_EVT: /* 18 */
            p_dev_cb = bta_hh_le_find_dev_cb_by_conn_id(p_data->cfg_mtu_cmpl.conn_id);
            if (p_dev_cb != NULL && p_data->cfg_mtu_cmpl.status == BTA_GATT_OK)
            {
                APPL_TRACE_DEBUG2("%s, + p_dev_cb->disc_flag = %d", __FUNCTION__, p_dev_cb->disc_flag );
                p_dev_cb->disc_flag |= BTA_HH_LE_MTU_CMPL;
                APPL_TRACE_DEBUG2("%s, ++ p_dev_cb->disc_flag = %d", __FUNCTION__, p_dev_cb->disc_flag );
                if(!(p_dev_cb->disc_flag & BTA_HH_LE_SEC_CMPL))
                    return;

                APPL_TRACE_DEBUG2("%s, p_dev_cb->total_srvc = %d", __FUNCTION__, p_dev_cb->total_srvc );
                if(p_dev_cb->total_srvc >0)
                    p_dev_cb->total_srvc = 0;
                bta_hh_le_pri_service_discovery(p_dev_cb);
                //waiting_for_MTU = FALSE;
            }
            break;
#endif
