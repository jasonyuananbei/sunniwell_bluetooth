...
void rtkbtAutopairCallback(byte len, byte[] data) {
        Log.d(TAG,"CALL rtkbtAutopairCallback: "+len);
/*
        if(data != null && len == data.length)
        {
            for(int i=0; i< len; i++)
                Log.d("rtkbtAutopairCallback","data " + data[i]);
        }
*/
        Intent intent = new Intent(BluetoothAdapter.ACTION_RTKBT_AUTOPAIR_EVENT);
        intent.putExtra(BluetoothAdapter.EXTRA_RTKBT_AUTOPAIR_DATA, data);
        intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
        mService.sendBroadcast(intent, mService.BLUETOOTH_PERM);
    }
