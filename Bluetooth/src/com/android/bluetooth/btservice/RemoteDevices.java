...
/* BOARD_HAVE_BLUETOOTH_RTK_AUTOPAIR Begin */
        private int mRTKBT_AUTOPAIR_Classic_Supported;
        private byte[] mRTKBT_AUTOPAIR_Address;
        private int mRTKBT_AUTOPAIR_RcuID;
        private int mRTKBT_AUTOPAIR_TxPower;
/* BOARD_HAVE_BLUETOOTH_RTK_AUTOPAIR End */

        DeviceProperties() {
            mBondState = BluetoothDevice.BOND_NONE;
            mRTKBT_AUTOPAIR_RcuID = 0;  /* BOARD_HAVE_BLUETOOTH_RTK_AUTOPAIR */
        }
...
/*BOARD_HAVE_RTK_BLUETOOTH_AUTOPAIR Begin*/
        /**
         * @return mRTKBT_AUTOPAIR_RcuID
         */
        int getRTKBT_AUTOPAIR_RcuID() {
            synchronized (mObject) {
                return mRTKBT_AUTOPAIR_RcuID;
            }
        }
        ...
        * @return
        */
        void setRTKBT_AUTOPAIR_RcuID(int id) {
            synchronized (mObject) {
            mRTKBT_AUTOPAIR_RcuID = id;
            }
        }
/*BOARD_HAVE_RTK_BLUETOOTH_AUTOPAIR End*/
                ...
                synchronized(mObject) {
                    switch (type) {
                        ...
                        case AbstractionLayer.BT_PROPERTY_REMOTE_RSSI:
                            ...
                            break;
/* BOARD_HAVE_BLUETOOTH_RTK_AUTOPAIR Begin */
                        case AbstractionLayer.BT_PROPERTY_RTKBT_AUTOPAIR_CLASSIC_SUPPORTED:
                            device.mRTKBT_AUTOPAIR_Classic_Supported = val[0];
                            debugLog("Classic Supported is:" + device.mRTKBT_AUTOPAIR_Classic_Supported);
                            break;
                        case AbstractionLayer.BT_PROPERTY_RTKBT_AUTOPAIR_CLASSIC_ADDR:
                            device.mRTKBT_AUTOPAIR_Address = val;
                            debugLog("Classic Remote Address is:" + Utils.getAddressStringFromByte(val));
                            BluetoothDevice ClassicDevice = getDevice(val);
                            DeviceProperties classicprop;
                            if (ClassicDevice == null) {
                                classicprop = addDeviceProperties(val);
                                ClassicDevice = getDevice(val);
                            } else {
                                classicprop = getDeviceProperties(ClassicDevice);
                            }
                            classicprop.mRTKBT_AUTOPAIR_RcuID = 1;
                            debugLog("Classic device address RCU id :" + classicprop.mRTKBT_AUTOPAIR_RcuID);
                            break;
                        case AbstractionLayer.BT_PROPERTY_RTKBT_AUTOPAIR_RCUID:
                            device.mRTKBT_AUTOPAIR_RcuID = Utils.byteArrayToInt(val);
                            debugLog("RCUID is:" + device.mRTKBT_AUTOPAIR_RcuID);
                            break;
                        case AbstractionLayer.BT_PROPERTY_RTKBT_AUTOPAIR_TXPOWER:
                            device.mRTKBT_AUTOPAIR_TxPower = val[0];    //TxPower from hal is in one byte
                            break;
/* BOARD_HAVE_BLUETOOTH_RTK_AUTOPAIR End */
                    }
                }
...
/* BOARD_HAVE_BLUETOOTH_RTK_AUTOPAIR Begin */
        intent.putExtra(BluetoothDevice.EXTRA_RTKBT_AUTOPAIR_CLASSICSUPPORTED, deviceProp.mRTKBT_AUTOPAIR_Classic_Supported);
        if(deviceProp.mRTKBT_AUTOPAIR_Classic_Supported != 0)
        {
            if(getDevice(deviceProp.mRTKBT_AUTOPAIR_Address) == null)
            {
                Log.d("AutoPairing", "deviceFoundCallback: RCU("+deviceProp.mRTKBT_AUTOPAIR_RcuID+":"+device+") Classic addr not found");
                return;
            }
            DeviceProperties classic_deviceProp = getDeviceProperties(getDevice(deviceProp.mRTKBT_AUTOPAIR_Address));
            classic_deviceProp.setRTKBT_AUTOPAIR_RcuID(1);
            intent.putExtra(BluetoothDevice.EXTRA_RTKBT_AUTOPAIR_CLASSICDEVICE, getDevice(deviceProp.mRTKBT_AUTOPAIR_Address));
        }
        intent.putExtra(BluetoothDevice.EXTRA_RTKBT_AUTOPAIR_RCUID, deviceProp.mRTKBT_AUTOPAIR_RcuID);
        intent.putExtra(BluetoothDevice.EXTRA_RTKBT_AUTOPAIR_TXPOWER, deviceProp.mRTKBT_AUTOPAIR_TxPower);
/* BOARD_HAVE_BLUETOOTH_RTK_AUTOPAIR End */
