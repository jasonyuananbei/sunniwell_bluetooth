...
static jmethodID method_rtkbtAutopairCallback;
#endif
...
#ifdef BLUETOOTH_RTK_AUTOPAIR
static void rtkbt_autopair_callback(uint8_t len, uint8_t* data) {
    jbyteArray evt_data;
    ...
    evt_data = callbackEnv->NewByteArray(lenth);
    if (!evt_data) {
        ALOGE("Fail to new jbyteArray autopair_event_callback");
        checkAndClearExceptionFromCallback(callbackEnv, __FUNCTION__);
        return;
    }

    callbackEnv->SetByteArrayRegion(evt_data, 0, lenth, (jbyte*)data);
    ALOGI("%s, CallVoidMethod1", __FUNCTION__);
    if(sJniCallbacksObj == NULL)
        ALOGI("%s, mCallbacksObj is NULL", __FUNCTION__);
    if(method_rtkbtAutopairCallback == NULL)
        ALOGI("%s, method_rtkbtAutopairCallback is NULL", __FUNCTION__);
    if(sJniCallbacksObj != NULL && method_rtkbtAutopairCallback != NULL)
    callbackEnv->CallVoidMethod(sJniCallbacksObj, method_rtkbtAutopairCallback, lenth, evt_data);

    ALOGI("%s, CallVoidMethod2", __FUNCTION__);
    checkAndClearExceptionFromCallback(callbackEnv, __FUNCTION__);
    callbackEnv->DeleteLocalRef(evt_data);
}

rtkbt_autopair_callbacks_t rtkbt_autopair_CallBacks = {
    sizeof(rtkbt_autopair_CallBacks),
    rtkbt_autopair_callback
};
#endif
...
#ifdef BLUETOOTH_RTK_AUTOPAIR
    method_rtkbtAutopairCallback = env->GetMethodID(jniCallbackClass, "rtkbtAutopairCallback","(B[B)V");
#endif
...
#ifdef BLUETOOTH_RTK_AUTOPAIR
        if ((ret = sBluetoothInterface->rtkbt_autopair_init(&rtkbt_autopair_CallBacks)) != BT_STATUS_SUCCESS) {
            ALOGE("Failed to initialize BluetoothAutoPair, ret: %d", ret);
            return JNI_FALSE;
        }else{
            ALOGE("Bluetooth init_autopair successfully");
        }
#endif
...
/*BOARD_HAVE_BLUETOOTH_RTK_3DD Begin*/
   ...
   if ((status = android::register_com_android_bluetooth_3dd(e)) < 0) {
       ALOGE("jni 3dd registration failure: %d", status);
      return JNI_ERR;
   }
#endif
